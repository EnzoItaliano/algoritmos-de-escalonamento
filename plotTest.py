...
def Run(processes):                             # função principal
    ...
    for i in processes_list:
        ...
        for j in range(len(i.block_starts)):
            print("Bloqueado " + str(i.block_starts[j]) + " Termina " + str(i.block_ends[j]))
    ...
    for i in processes_list:
        ...
        temp_starts.pop(0)
        temp_ends.pop(len(temp_ends)-1)
        if i.incoming != i.starts[0]:
        ...
        for j in range(len(i.starts)):
            temp_pairs.append(i.ends[j] - i.starts[j])

        for j in range(len(i.block_starts)):
            temp_block.append(i.block_ends[j] - i.block_starts[j])

        for j in range(len(temp_starts)):
            temp_wait.append(temp_starts[j] - temp_ends[j])

        pairs.append(list((zip(i.starts,temp_pairs))))
        block_time_plot.append(list((zip(i.block_starts,temp_block))))
        wait_time_plot.append(list((zip(temp_ends,temp_wait))))
    ...
    print(wait_time_plot[0])

    fig, ax = plt.subplots()
    for i in range(len(pairs)):
        ax.broken_barh(pairs[i], ((i+1), 1), color=(random.uniform(0, .9), random.uniform(0, 1), random.uniform(0, 1)))

    for i in range(len(wait_time_plot)):    
        ax.broken_barh(wait_time_plot[i], ((i+1), 1), color=(0,0,0),alpha=0.1)

    for i in range(len(block_time_plot)):
    ...
    ax.set_yticks(np.arange(0, len(processes_list)+2, 1))
